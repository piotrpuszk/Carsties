...
namespace AuctionService.UnitTests
{
  public class AuctionControllerTests
  {
    ...
    [Fact]
    public async Task GetAuctionById_InvalidGuid_ReturnsNotFound()
    {
      //arrange
    ...
    {
      var auctionCreateDto = _fixture.Create<CreateAuctionDto>();
      _auctionRepo.Setup(repo => repo.AddAuction(It.IsAny<Auction>()));
      _auctionRepo.Setup(repo => repo.SaveChangesAsync()).ReturnsAsync(false);

      var result = await _controller.CreateAuction(auctionCreateDto);
    ...
    {
      var auction = _fixture.Build<Auction>().Without(e => e.Item).Create();
      auction.Item = _fixture.Build<Item>().Without(e => e.Auction).Create();
      auction.Seller = "test";
